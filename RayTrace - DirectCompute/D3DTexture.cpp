...
void D3DTexture::createTexture(std::wstring* p_pTextureFilename, char* p_pDebugName, bool dds)
{
		...
		if(!dds)
		{
			...
			if(SUCCEEDED(DirectX::CreateDDSTextureFromFile(m_pDevice, temp, (ID3D11Resource**)m_pResource, &m_pResourceView, 0U, nullptr)))
			{
					//m_pResourceView = createTextureSRV(m_pResource);
